...
$$
f(\beta, \sigma^2|y,x) = \frac{f_y(y|x, \beta,\sigma^2)f(\beta|\sigma^2,m,V)f(\sigma^2|a,b)}{f_y(y)}
$$
...
$$
f(\beta|y, X, \sigma^2) ∝ f_y(y|X, \beta)\prod_{i=1}^{n}f(\beta_i|m_i,v_i) 
$$
...
We select correlation value 0.8 as the threshold of multicollinearity.
Any value greater than 0.8 will be viewed as the presence of
multicollinearity. As observed the correlation matrix, there are no
values greater than 0.8,which means that there is no multicollinearity
